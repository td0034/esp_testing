...
const char* serverName = "http://emdrtool.ddns.net/post-esp-data.php";
const char* GET_server = "http://emdrtool.ddns.net/get_slider_data.php";
...
const unsigned long tapperIntervalMax = 2000; // Interval for turning off the device (1 second)
...
//Ported to ESP32
  ...
  */
  ...
  #else
    const int TRIGGER_PIN = PIN_D9;   // Pin D0 mapped to pin GPIO0/BOOT/ADC11/TOUCH1 of ESP32
    /*
    Alternative trigger pin. Needs to be connected to a button to use this pin. It must be a momentary connection
    not connected permanently to ground. Either trigger pin will work.
    */
...
String ssid = "EMDR Tool " + String(ESP_getChipId(), HEX);
String password;
